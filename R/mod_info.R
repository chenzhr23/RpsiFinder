...
#' @export
#'
mod_info <- function(RpsiFinder_res_file,
                     ...
                     output_name)
{
  ...
  modinfo<-paste(output_dir,"/",basename(sub(".bed","_modinfo.txt",RpsiFinder_res_file)),sep="")
  ...
  print(cmd1)
  system(cmd1)
  assign(paste(output_name,"_RpsiAnnotator_res_file6_R",sep=""),fread(RpsiAnnotator_res_file6),envir = .GlobalEnv)
  ...
  print(cmd2)
  system(cmd2)
  assign(paste(output_name,"_RpsiAnnotator_res_file6.append_R",sep=""),fread(RpsiAnnotator_res_file6.append),envir = .GlobalEnv)

  cmd3 <- paste(bedtools_path,
                "intersect",
                "-a", RpsiFinder_res_file,
  ...
  if(pred_method=="roc"){
    cmd4 <- paste(bedtools_path,
                  ...
                  "-wb|awk \'FS=OFS=\"\t\" {print \"\"$1\"_\"$2\"_\"$3\"_\"$6\"\",$44,$39}\' >",#ann prediction result
                  known.target,
                  sep=" ")
    ...
    cmd4 <- paste(bedtools_path,
                  ...
                  "-a", anno_info.bed,
                  "-b", psiU.SingleSites.bed,
                  "-wb|awk \'FS=OFS=\"\t\" {print \"\"$1\"_\"$2\"_\"$3\"_\"$6\"\",$49,$39}\' >",#ann prediction result
                  known.target,
                  sep=" ")
    ...
    cmd4 <- paste(bedtools_path,
                  ...
                  "-a", anno_info.bed,
                  "-b", psiU.SingleSites.bed,
                  "-wb|awk \'FS=OFS=\"\t\" {print \"\"$1\"_\"$2\"_\"$3\"_\"$6\"\",$46,$39}\' >",#ann prediction result
                  known.target,
                  sep=" ")
  }else{
    print(paste("Error file input: ",RpsiFinder_res_file,sep=""))
  }
  ...
  cmd5 <- paste(bedtools_path,
                ...
                "-a", anno_info.bed,
                "-b", psiU.SingleSites.bed,
                "-wb -v |awk \'FS=OFS=\"\t\" {print \"\"$1\"_\"$2\"_\"$3\"_\"$6\"\",\"unknown\",$39}\' >",
                unknown.target,
                sep=" ")
  print(cmd5)
  system(cmd5)
