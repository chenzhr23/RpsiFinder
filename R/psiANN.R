...
psiANN <- function(rocfile, rRNAfile, rRNAfile2, filtfile,output_dir,output_name)
{
  ...
  print(table(ANN_data_sel$class))
  ...
  print(coords(ann_roc_test, "best", ret = "all", transpose = TRUE)[1])

  print(summary(ANN_data_sel.net))
  print(str(ANN_data_sel.net))
  ...
  ANN_data_evidence<-ANN_data %>% left_join(evidence,by=c("rRNA_uniq_id"="rRNA_uniq_id")) %>% as_tibble() %>% filter(!is.na(rRNA_anno))
  ...
  ANN_data$rRNA_uniq_id<-paste(ANN_data$chrom,ANN_data$chromStart,ANN_data$chromEnd,ANN_data$strand,sep="_")
  ANN_data_evidence2<-ANN_data %>% left_join(evidence2,by=c("rRNA_uniq_id"="rRNA_uniq_id")) %>% as_tibble() %>% filter(!is.na(rRNA_anno))
